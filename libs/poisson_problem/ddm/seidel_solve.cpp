...
SeidelSolve::SeidelSolve(const Grid& grid, size_t overlap, double tolerance, const std::string& glue_strategy,
                         size_t maxit)
...
void SeidelSolve::solve(std::vector<double>& u, size_t& iters) {
    ...
    throw std::runtime_error("Seidel did not converge in " + std::to_string(maxit_) + " iterations");
}